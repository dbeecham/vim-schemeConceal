...
" Last Change:  2014 Aug 01
" Maintainer:   Daniel Beecham <daniel@lunix.se>
" Original authors: Dirk van Deun <dirk@igwe.vub.ac.be>,
"                   Sergey Khorev <sergey.khorev@gmail.com>

" This file is a modified version of the work by Sergey and Dirk
" to allow for the use of conceal.
...
syn match	schemeOther	,[a-z!$%&*/:<=>?^_~+@#%-][-a-z!$%&*/:<=>?^_~0-9+.@#%]*, contains=schemeConceal
...
syn keyword schemeFunc not boolean? eq? eqv? equal? pair? cons car cdr
syn keyword schemeFunc caar cadr cdar cddr caaar caadr cadar caddr 
...
syn keyword schemeFunc real-part imag-part magnitude angle 
...
syn keyword schemeFunc symbol? number? complex? 
syn keyword schemeFunc char-upcase  char-downcase
...
syn keyword schemeFunc vector vector-length vector-ref procedure? 
...
syn keyword schemeFunc list-tail string-copy 
...
syn keyword schemeFunc hashtable? hashtable-size hashtable-ref 
syn keyword schemeFunc hashtable-contains? 
syn keyword schemeFunc hashtable-copy hashtable-keys 
...
syn match schemeFunc "char->integer" contains=schemeConcealFunction
syn match schemeFunc "symbol->string" contains=schemeConcealFunction
syn match schemeFunc "string->symbol" contains=schemeConcealFunction
syn match schemeFunc "integer->char" contains=schemeConcealFunction
syn match schemeFunc "vector->list" contains=schemeConcealFunction
syn match schemeFunc "list->vector" contains=schemeConcealFunction
syn match schemeFunc "number->string" contains=schemeConcealFunction
syn match schemeFunc "string->number" contains=schemeConcealFunction
syn match schemeFunc "exact->inexact" contains=schemeConcealFunction
syn match schemeFunc "inexact->exact" contains=schemeConcealFunction
syn match schemeFunc "inexact->exact" contains=schemeConcealFunction
syn match schemeFunc "list->string" contains=schemeConcealFunction

syn match schemeFunc "set-cdr!" contains=schemeMutable
syn match schemeFunc "set-car!" contains=schemeMutable
syn match schemeFunc "vector-set!" contains=schemeMutable
syn match schemeFunc "string-fill!" contains=schemeMutable
syn match schemeFunc "vector-fill!" contains=schemeMutable
syn match schemeFunc "hashtable-set!" contains=schemeMutable
syn match schemeFunc "hashtable-update!" contains=schemeMutable
syn match schemeFunc "hashtable-clear!" contains=schemeMutable
syn match schemeFunc "hashtable-delete!" contains=schemeMutable
...
if !exists('g:no_scheme_highlight_mutable')
    syntax match schemeMutable ".*!\>" contains=schemeMutableBang
    syntax match schemeMutableBang "!" contained conceal transparent
    highlight default link schemeMutable Underlined
    setlocal conceallevel=2
endif

" Scheme concealement
if !exists('g:no_scheme_conceal') && has('conceal') && &enc == 'utf-8'
    syntax match schemeConcealFunction "<-" conceal cchar=← contained
    syntax match schemeConcealFunction "->" conceal cchar=→ contained

    syntax match schemeConceal "<-" conceal cchar=←
    syntax match schemeConceal "->" conceal cchar=→

    syntax keyword schemeConcealFunction not conceal cchar=¬
    syntax keyword schemeConcealFunction sum conceal cchar=∑
    syntax keyword schemeConcealFunction product conceal cchar=∏
    syntax keyword schemeConcealFunction sqrt conceal cchar=√
    syntax keyword schemeConcealFunction forall conceal cchar=∀
    syntax keyword schemeConcealFunction exists? conceal cchar=∃
    syntax keyword schemeConcealFunction compose conceal cchar=∘
    syntax keyword schemeConcealConstant pi conceal cchar=π
    syntax keyword schemeConcealConstant tau conceal cchar=τ
    syntax keyword schemeConcealSyntax lambda conceal cchar=λ
    syntax keyword schemeConcealSyntax >= conceal cchar=≥
    syntax keyword schemeConcealSyntax <= conceal cchar=≤

    highlight link schemeConcealFunction schemeFunc
    highlight link schemeConcealConstant schemeConstant
    highlight link schemeConcealSyntax schemeSyntax
